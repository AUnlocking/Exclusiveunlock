...
class Scroll extends BaseController{
	...
	public function loadscroll(){
		$data['start'] 		= ($this->request->getVar('start')) ? $this->request->getVar('start'): 0;
		$data['limit'] 		= ($this->request->getVar('limit')) ? $this->request->getVar('limit'): 8;
		$data['users'] 		= $this->request->getVar('users')? $this->request->getVar('users'): "";
		$data['arts'] 		= $this->request->getVar('arts') ? $this->request->getVar('arts'): "";
		$data['slug'] 		= $this->request->getVar('slug') ? $this->request->getVar('slug'): "";
		$data['search'] 	= $this->request->getVar('search') ? $this->request->getVar('search'): "";
		$data['id_usuario'] = $this->request->getVar('id_usuario') ? $this->request->getVar('id_usuario'): 0;
		$data['order_by'] 	= ($this->request->getVar('order_by')) ? $this->request->getVar('order_by'): "nom_post";
		...
		foreach ($posts as $post) {
			...
			$response['data'] .= '<div class="col-lg-4 col-md-4 col-sm-6 col-xs-6 mb-2 d-flex">
				<div class="card mb-2 post border-bottom-2" style="width:100%;">
						<a href="'.base_url('post/'.$post->slug_post).'">
							<div class="post-image rounded">
								<img src="'.base_url($img).'" class="" alt="" sizes="(max-width: 272px) 100vw, 272px">
							</div>
						</a>
